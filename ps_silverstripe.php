...
task('sitehost:upgrade-mysql', function () {
    ...
    run("cd {{shared_path}} && sed 's/\\r\$//' .env > .env.crlf && mv .env.crlf .env && export $(grep -v '^#' .env | xargs) && mysqldump -u \$SS_DATABASE_USERNAME -p\$SS_DATABASE_PASSWORD -h \$SS_DATABASE_SERVER --column-statistics=0 --no-tablespaces \$SS_DATABASE_NAME > {{upgrade_path}}/mysql57-backup.sql");
